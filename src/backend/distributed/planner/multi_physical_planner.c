...
static Job * BuildJobTreeTaskList(Job *jobTree, RelationRestrictionContext *restrictionContext);
static List * SubquerySqlTaskList(Job *job, RelationRestrictionContext *restrictionContext);
static Task *
 ...
 */
MultiPlan *
MultiPhysicalPlanCreate(MultiTreeRoot *multiTree, Query *originalQuery,
						RelationRestrictionContext *restrictionContext)
{
...
SubquerySqlTaskList(Job *job,RelationRestrictionContext *restrictionContext)
{
	...
	DistTableCacheEntry *targetCacheEntry = DistributedTableCacheEntry(relationId);
								 ...
								 uint32 taskIdIndex)
{
	...
	foreach(restrictionCell, copiedRestrictionContext->relationRestrictionList)
	{
		...
		ShardInterval *relationShard = NULL;


		//elog (INFO, "Relation: %s", get_rel_name(restriction->relationId));
		//elog (INFO, "Index: %d", restriction->index);
		//elog (INFO, "RTE %s", pretty_format_node_dump(nodeToString(restriction->rte)));


		foreach(shardIntervalCell, colocatedShardList)
		{
			ShardInterval *tmp = lfirst(shardIntervalCell);

			if (tmp->relationId == restriction->relationId)
			{
				relationShard = tmp;
				break;
			}
		...
		instantiateQualWalker->targetShardInterval = relationShard;
//elog(INFO, "Starting to instantiate for relation :%s", get_rel_name(relationShard->relationId));

//elog(DEBUG1, "originalBaserestrictInfo: %s", nodeToString(originalBaserestrictInfo));
//elog(DEBUG1, "originalJoinInfo: %s", nodeToString(originalJoinInfo));
	...
	copiedQuery->jointree->quals = (Node *) make_ands_explicit((List *) copiedQuery->jointree->quals);;
	...
	modifyTask = CreateBasicTask(jobId, taskIdIndex, SQL_TASK, queryString->data);
	modifyTask->dependedTaskList = NULL;
	modifyTask->anchorShardId = selectAnchorShardId;
	modifyTask->taskPlacementList = selectPlacementList;
